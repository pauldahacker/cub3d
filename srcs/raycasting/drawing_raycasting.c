...
/*   Updated: 2024/11/14 19:02:26 by simarcha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_every_ray(t_vars *vars)
{
	...
	player->proj_plan.length_column = round(WINDOW_X / PROJ_PLANE_X);
	player->proj_plan.center_pp_y = PROJ_PLANE_Y / 2;
	player->proj_plan.distance_player_pplan = ((PROJ_PLANE_X / 2)
			/ tan((FOV / 2) * PI / 180.0));
	x = 0;
	y = 0;
