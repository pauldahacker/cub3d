...
/*   Updated: 2024/10/31 19:51:10 by simarcha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	finding_vertical_y_a(t_vars *vars)
{
	double	y_a_iteration;

	(void)vars;
	y_a_iteration = ft_abs((double)BLOCK_SIZE * vars->game->player->tan_result);
	if (ray_facing_up(vars->game->player->ray_angle) == 1)
		return (-y_a_iteration);
	return (y_a_iteration);
}

static t_block	find_next_vertical_point(t_vars *vars, t_block current_point)
{
	t_block	next_in_px;
	double	x_a;
	double	y_a;

	x_a = finding_vertical_x_a(vars->game->player->ray_angle);
