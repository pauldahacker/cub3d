...
/*   Updated: 2024/11/11 20:26:27 by simarcha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_texture(t_vars *vars, int x, int *y, t_texture tex)
{
	...
	if (player->block_touched.reachable == false)//it means that this is the first ray and block_touched is not set yet
		player->block_touched = player->next_block_touched;//we set it
	...
	if (rounded_down(player->block_touched.x) == rounded_down(player->next_block_touched.x)
			&& rounded_down(player->block_touched.y) == rounded_down(player->next_block_touched.y))
	...
	if (!(rounded_down(player->block_touched.x) == rounded_down(player->next_block_touched.x)))
	{
		player->proj_plan.wall_upper_left_x_in_px = player->block_touched.x;
		player->proj_plan.wall_upper_right_x_in_px = player->next_block_touched.x;
		player->horizontal_wall_size_in_px = player->proj_plan.wall_upper_right_x_in_px - player->proj_plan.wall_upper_left_x_in_px;
		printf("EN XXXXXX: \nplayer->block_touched.x = %f\n", player->block_touched.x);
		printf("player->next_block_touched.x = %f\n", player->next_block_touched.x);
		printf("player->horizontal_wall_size_in_px = %f\n", player->horizontal_wall_size_in_px);
	}
	else if (!(rounded_down(player->block_touched.y) == rounded_down(player->next_block_touched.y)))
	{
		player->proj_plan.wall_upper_left_x_in_px = player->block_touched.x;
		player->proj_plan.wall_upper_right_x_in_px = player->next_block_touched.x;
		player->horizontal_wall_size_in_px = player->proj_plan.wall_upper_right_x_in_px - player->proj_plan.wall_upper_left_x_in_px;
	...
	player->block_touched = player->next_block_touched;
}
