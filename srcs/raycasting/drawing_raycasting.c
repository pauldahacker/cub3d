...
/*   Updated: 2024/11/14 19:31:21 by simarcha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*static double calculate_projected_wall_height(t_vars *vars)
{
	...
	projected_wall_height = (actual_wall_height / player->distance_to_wall) * player->proj_plan.distance_player_pplan;
...
static void set_calculus_projection_plan(t_proj *proj_plan,
										 double projected_wall_height)
{
	proj_plan->wall_top_pos_y_in_pp = proj_plan->center_pp_y - projected_wall_height / 2;
	proj_plan->wall_lower_pos_y_in_pp = proj_plan->center_pp_y + projected_wall_height / 2;
	proj_plan->wall_top_pos_y_in_px = proj_plan->wall_top_pos_y_in_pp * WINDOW_Y / PROJ_PLANE_Y;
	proj_plan->wall_lower_pos_y_in_px = proj_plan->wall_lower_pos_y_in_pp * WINDOW_Y / PROJ_PLANE_Y;
}
	...
	if (*x < WINDOW_X)
	{
		*y = 0;
		while (*y < WINDOW_Y)
		{
			...
			else if (*y >= proj_plan.wall_top_pos_y_in_px && *y <= proj_plan.wall_lower_pos_y_in_px)
				draw_wall(vars, x, y);
...
{
	vars->game->player->proj_plan.length_column = round(WINDOW_X / PROJ_PLANE_X);
	vars->game->player->proj_plan.center_pp_y = PROJ_PLANE_Y / 2;
	vars->game->player->proj_plan.distance_player_pplan = ((PROJ_PLANE_X / 2) / tan((60 / 2) * PI / 180.0));
}
