...
/*   Updated: 2024/10/31 19:50:37 by simarcha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	finding_horizontal_x_a(t_vars *vars)
{
	double	x_a_iteration;

	(void)vars;
	x_a_iteration = ft_abs((double)BLOCK_SIZE / vars->game->player->tan_result);
	if (ray_facing_right(vars->game->player->ray_angle) == 1)
		return (x_a_iteration);
	return (-x_a_iteration);
}

static t_block	find_next_horizontal_point(t_vars *vars, t_block current_point)
{
	t_block	next_in_px;
	double	x_a;
	double	y_a;
	...
	y_a = finding_horizontal_y_a(vars->game->player->ray_angle);
