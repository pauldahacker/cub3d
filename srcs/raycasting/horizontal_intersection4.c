...
t_block	check_horizontal_first_pt(t_vars *vars, double ray_angle)
{
	...
	first_intersected_point.x = vars->game->player->pos_x + (vars->game->player->pos_y - first_intersected_point.y)
...
double	horizontal_x_a(double ray_angle)
{
	...
	if (ray_angle == 0.000000)
		ray_angle += 0.000001;
	x_a_iteration = ft_abs(rounded_down(BLOCK_SIZE / tan(ray_angle * PI / 180.0)));//maybe here you can rounded_down
